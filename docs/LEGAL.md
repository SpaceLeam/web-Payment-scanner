# Legal Notice and Disclaimer

## ⚖️ Terms of Use

This software ("Web Payment Scanner") is provided for **legitimate security testing purposes only**.

### ✅ Authorized Use

You MAY use this tool for:

1. **Security Testing with Permission**
   - Testing applications you own
   - Testing with explicit written authorization
   - Bug bounty programs within defined scope
   - Authorized penetration testing engagements

2. **Educational Purposes**
   - Learning about payment security
   - Security research in controlled environments
   - Training and certification programs

3. **Defensive Security**
   - Testing your own security controls
   - Validating security implementations
   - Compliance testing (PCI-DSS, etc.)

### ❌ Prohibited Use

You MAY NOT use this tool for:

1. **Unauthorized Access**
   - Testing systems without permission
   - Bypassing authentication or authorization
   - Accessing data you're not authorized to view

2. **Financial Fraud**
   - Manipulating payment amounts for personal gain
   - Stealing payment information
   - Creating unauthorized transactions

3. **Malicious Activities**
   - Causing financial harm to businesses
   - Disrupting payment services
   - Data theft or exfiltration

4. **Illegal Activities**
   - Any violation of local, state, or federal laws
   - Violation of computer fraud and abuse laws
   - Breach of terms of service

---

## 🚨 Legal Consequences

Unauthorized use of this tool may violate:

- **Computer Fraud and Abuse Act (CFAA)** (USA)
- **Computer Misuse Act** (UK)
- **European Union Cybercrime Directive**
- **Payment Card Industry Data Security Standard (PCI-DSS)**
- Local cybercrime laws in your jurisdiction

**Penalties may include**:
- Criminal prosecution
- Civil lawsuits
- Significant fines
- Imprisonment
- Permanent criminal record

---

## 📋 Responsible Disclosure

If you discover vulnerabilities using this tool:

1. ✅ **DO** report them through proper channels:
   - Bug bounty programs
   - Security@company.com emails
   - Coordinated disclosure platforms

2. ✅ **DO** provide sufficient detail for reproduction:
   - Steps to reproduce
   - Proof of concept (without causing harm)
   - Suggested remediation

3. ❌ **DO NOT**:
   - Publicly disclose before patch
   - Sell vulnerabilities to malicious actors
   - Exploit vulnerabilities for personal gain
   - Access more data than necessary to prove the issue

---

## 🛡️ Disclaimer

### No Warranty

This software is provided **"AS IS"** without warranty of any kind, express or implied, including but not limited to:
- Merchantability
- Fitness for a particular purpose
- Non-infringement

### Limitation of Liability

The authors and contributors shall **NOT** be liable for:
- Any damages arising from use or misuse of this software
- Any illegal activities conducted with this tool
- Any financial losses or security breaches
- Any consequence of unauthorized testing

### User Responsibility

**YOU** are solely responsible for:
- Obtaining proper authorization before testing
- Compliance with applicable laws
- Any consequences of your actions
- Proper handling of discovered vulnerabilities

---

## 📝 Authorization Checklist

Before using this tool, ensure you have:

- [ ] **Written permission** to test the target system
- [ ] **Defined scope** of what can be tested
- [ ] **Clear timeline** for testing activities
- [ ] **Emergency contact** information
- [ ] **Non-disclosure agreement** (if applicable)
- [ ] **Understanding of rules of engagement**

---

## 🔒 Best Practices

1. **Always Get Permission**
   - Written authorization is best
   - Email confirmation at minimum
   - Document scope clearly

2. **Test Responsibly**
   - Don't cause service disruption
   - Don't access unnecessary data
   - Stop if you detect impact

3. **Report Professionally**
   - Use proper disclosure channels
   - Provide clear documentation
   - Allow reasonable time for fixes

4. **Protect Findings**
   - Don't share vulnerabilities publicly prematurely
   - Keep proof-of-concept private
   - Follow disclosure timelines

---

## 📞 Contact

For questions about legal use:
- Open an issue on GitHub (for general questions)
- Consult with a cybersecurity attorney (for legal advice)
- Review bug bounty program rules (for specific platforms)

---

## ✍️ Acceptance

**By using this tool, you acknowledge that:**

1. You have read and understood this legal notice
2. You agree to use this tool only for authorized purposes
3. You accept full responsibility for your actions
4. You understand the potential legal consequences of misuse

**If you do not agree to these terms, DO NOT use this software.**

---

*Last Updated: November 2025*

**Remember: Ethical hacking is about making the internet safer, not exploiting it for personal gain. Always act with integrity.** 🛡️
